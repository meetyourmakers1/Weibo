...
$(function () {
    ...
    $('.add-group-sub').click(function (){
        ...
        if(groupName != ''){
            $.post(addGroupUrl,{name : groupName},function(data){
                if(data.status){
                    showTips(data.msg);
                    $('#add-group').hide();
                    $('#group-bg').remove();
    ...
    $('.add-fl').click(function () {
        ...
        $('input[name=follow]').val($(this).attr('userid'));
    });
    //异步添加关注
    $('.add-follow-sub').click(function () {
        var follow = $('input[name=follow]').val();
        var group = $('select[name=groupid]').val();
        $.post(addFollow,{
            'group' : group,
            'follow' : follow
        },function(data){
            if(data.status){
                showTips(data.msg);
                $('.add-fl[userid=' + follow + ']').removeClass('add-fl').html('√&nbsp;已关注');
                $('#follow').hide();
                $('#follow-bg').remove();
            }else{
                alert(data.msg);
    ...
    * */
    $('#model .model_save').click(function(){
        ...
        if(!theme){
            ...
            $.post(styleUrl,{style : theme},function(data){
                if(data){
